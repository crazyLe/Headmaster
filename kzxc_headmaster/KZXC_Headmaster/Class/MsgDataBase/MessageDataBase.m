...
-(NSArray *)queryCircleMessage
{
    ...
    [queue inDatabase:^(FMDatabase *db) {
        NSString *sql =  @"select * from messageTable where( msg_id=39 or msg_id=40  )ORDER BY id DESC ";
        FMResultSet *resultSet =  [db executeQuery:sql];
        // 遍历结果集合
        while ([resultSet next])
        {
...
-(NSArray *)queryCircleUnRead
{
    ...
    [queue inDatabase:^(FMDatabase *db) {
        NSString *sql =  @"select * from messageTable where (msg_id between 39 and 40) and isRead=0 ORDER BY id DESC ";
        FMResultSet *resultSet =  [db executeQuery:sql];
        // 遍历结果集合
        while ([resultSet next])
        {
