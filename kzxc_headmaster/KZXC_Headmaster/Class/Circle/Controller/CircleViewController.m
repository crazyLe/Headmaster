...
@property(nonatomic,assign)int platFormPageId;

@property(nonatomic,assign)int studentPageId;

@property(nonatomic,copy)NSString * commityType;
...
- (void)msgRead
{
    ...
    self.navigationItem.rightBarButtonItem.badgeValue = [NSString stringWithFormat:@"%lu",num];
...
-(void)configNav
{
    ...
    [button setImage:[UIImage imageNamed:@"circlre_msg"] forState:UIControlStateNormal];
...
-(void)seeMessage
{
    if (kLoginStatus) {
        ...
        SystemDetailsvController *msgVC = [[SystemDetailsvController alloc]init];
...
//    NSString *url = self.interfaceManager.getCommunity;
    ...
    [HJHttpManager PostRequestWithUrl:getCircleUrl param:param finish:^(NSData *data) {
...
- (void)createUI
{
    ...
    [self.sendMessgaeBtn setImage:[UIImage imageNamed:@"circle_send"] forState:UIControlStateNormal];
...
-(void)sendMessageBtnClick
{
    if ([kUid intValue] != 0) {
        ...
        [root.navigationBar setBackgroundImage:[UIImage imageNamed:@"nav_bg"] forBarMetrics:UIBarMetricsCompact];
        root.navigationBar.shadowImage =[UIImage imageNamed:@"nav_bg"];
        root.navigationBar.titleTextAttributes = NavTitleTextAttributes;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == _platFormTableView) {
        ...
        circleDetail.urlString = [NSString stringWithFormat:@"%@/%d?app=1&uid=%@&cityId=%lu&address=%@,%@",basicCircleWebUrl,mainModel.circleMainModel.idNum,kUid,[HttpParamManager getCurrentCityID],[HttpParamManager getLongitude],[HttpParamManager getLatitude]];
