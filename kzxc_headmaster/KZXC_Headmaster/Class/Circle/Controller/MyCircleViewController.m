...
//    [self createNavWithLeftBtnImageName:@"返回" leftHighlightImageName:nil leftBtnSelector:@selector(leftBtnClick) andCenterTitle:nil];
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageWithRenderingMode:@"back"] style:UIBarButtonItemStylePlain target:self action:@selector(leftBtnClick)];
...
//    NSString *url = self.interfaceManager.memberCommunity;
    ...
    [HJHttpManager PostRequestWithUrl:getMyCircleUrl param:param finish:^(NSData *data) {
...
//    NSString *url = self.interfaceManager.memberCommunityList;
    ...
    [HJHttpManager PostRequestWithUrl:getMyCircleListUrl param:param finish:^(NSData *data) {
        NSDictionary *dict = [NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
        NSLog(@"%@",dict);
        NSInteger code = [dict[@"code"] integerValue];
        NSString *msg = dict[@"msg"];
...
{
    NSString *url = self.segmentControl.selectedSegmentIndex==0?getMyCircleUrl:getMyCircleListUrl;
    NSString *time = [HttpParamManager getTime];
    NSMutableDictionary *param = [NSMutableDictionary dictionary];
    param[@"uid"] = kUid;
    param[@"time"] = time;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *componentStr = [NSString stringWithFormat:@"/%@?uid=%@&app=1&cityid=%ld&address=%@,%@",momentId,kUid,[HttpParamManager getCurrentCityID],[HttpParamManager getLongitude],[HttpParamManager getLatitude]];
    
    CircleDetailWebController *circleDetail  =[[CircleDetailWebController alloc]init];
    circleDetail.urlString = [NSString stringWithFormat:@"%@%@",basicCircleWebUrl,componentStr];
