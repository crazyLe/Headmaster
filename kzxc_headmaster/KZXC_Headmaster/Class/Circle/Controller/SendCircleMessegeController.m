...
//    NSString *url = self.interfaceManager.createCommunity;
    ...
    [HJHttpManager PostRequestWithUrl:sendMyCircleUrl param:param finish:^(NSData *data) {
...
-(void)uploadPictureWithCircleID:(int)idNum
{
    ...
    for (int i = 0; i<self.imageArray.count; i++) {
        ...
        [HJHttpManager UploadFileWithUrl:uploadCirclePicsUrl param:paramDic serviceName:@"pic" fileName:@"0.png" mimeType:@"image/jpeg" fileData:imageData finish:^(NSData *data) {
            NSDictionary * dict = [NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
            NSLog(@"+++++%@",dict);
            NSInteger code = [dict[@"code"] integerValue];
            NSString * msg = dict[@"msg"];
...
-(void)createUI
{
    ...
    NSLog(@"%f",self.textView.innerTextViewBottomConstriant.constant);
