...
#define WJSegmentMenuVcDefaultButtonFont        [UIFont systemFontOfSize:20]
#define WJSegmentMenuVcDefaultUnselectedColor   [UIColor darkGrayColor]
#define WJSegmentMenuVcDefaultSelectedColor     [UIColor colorWithHexString:@"#ff6866"]
#define WJSegmentMenuVcDefaultSlideColor        [UIColor colorWithHexString:@"#ff6866"]
...
- (void)addSubVc:(NSArray *)vc subTitles:(NSArray *)titles{
    ...
    artcleArr = [NSMutableArray array];
    [self loadDatatype:0 andpage:1];
}
...
//        - WJSegmentMenuVcDefaultSpace*2
        UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake(WJSegmentMenuVcDefaultSpace + (WJSegmentMenuVcDefaulButtonW + WJSegmentMenuVcDefaultSpace) * i, 0, WJSegmentMenuVcDefaulButtonW, self.frame.size.height )];
...
//                view.layer.masksToBounds = YES;
            ...
            if (self.MenuVcSlideType == WJSegmentMenuVcSlideTypeSlide) {
                ...
                TempFrame.origin.y = CGRectGetMaxY(btn.frame)-2;
                UIView *view = [[UIView alloc]initWithFrame:TempFrame];
                view.backgroundColor = [UIColor colorWithHexString:@"#ff6866"];
//                view.layer.cornerRadius = 10;
//                view.layer.masksToBounds = YES;
                [scrollView addSubview:view];
                self.view = view;
    ...
    [NOTIFICATION_CENTER addObserver:self selector:@selector(refreshtable:) name:@"articleLoadMore" object:nil];

    [NOTIFICATION_CENTER addObserver:self selector:@selector(refreshZan) name:@"refreshZanNum" object:nil];
}

-(void)refreshZan
{
     [artcleArr removeAllObjects];
     [self loadDatatype:clickedbtnIndex andpage:1];
}
...
- (void)segmentMenuTitleClick:(UIButton *)btn{
    ...
    NSLog(@"%d",(int)btn.tag - 960);
    [artcleArr removeAllObjects];
    NSInteger clickedIndex = btn.tag - tag;
    clickedbtnIndex = clickedIndex;
    [self loadDatatype:clickedIndex andpage:1];
...
- (void)refreshtable:(NSNotification *)notification
{
    ...
    NSInteger index = [userInfo[@"curpage"] integerValue];
    UITableView *table = userInfo[@"table"];
    [self loadDatatype:clickedbtnIndex andpage:index+1];
    [table.mj_footer endRefreshing];
}

- (void)loadDatatype:(NSInteger)type andpage:(NSInteger)page
{
    ...
    param[@"typeId"] = @(type);
    param[@"pageId"] = @(page);
    param[@"sign"] = [HttpsTools getSignWithIdentify:@"/article" time:self.getcurrentTime];
    NSLog(@"%@",param);
    [HttpsTools kPOST:masterLessonUrl parameter:param progress:^(NSProgress *downloadProgress) {
    } succeed:^(id backdata, int code, NSString *msg) {
        NSLog(@"%@=======%d===%@",backdata,code,msg);
        if (1 == code) {
            NSArray *articles = backdata[@"article"];
            if (articles.count == 0 && 1 != page) {
                [self.hudManager showSuccessSVHudWithTitle:@"没有更多文章了" hideAfterDelay:2.0 animaton:YES];
                return ;
            }
            [self.hudManager dismissSVHud];
            for (int j = 0; j<articles.count; j++) {
                ArticleModel *model = [ArticleModel mj_objectWithKeyValues:articles[j]];
                [artcleArr addObject:model];
            }
            LessonTypeController *lesson = self.vcArray[type];
            lesson.lessview.modelarr = artcleArr;
            [lesson.lessview.lessonTable reloadData];
           
            if (1 == page) {
                bannerArr = [BannerModel mj_objectArrayWithKeyValuesArray:backdata[@"banner"]];
                [lesson.lessview.bannerView refreshData: bannerArr];
            }
        }
