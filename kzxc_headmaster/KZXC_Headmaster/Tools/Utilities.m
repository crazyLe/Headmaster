...
+ (NSString *)calculateTimeWithFaceTimeDayTarget:(NSString *)dateString
{
    ...
    if (dateComponents.year == currentDateComponents.year && dateComponents.month == currentDateComponents.month && dateComponents.day == currentDateComponents.day){
        ...
        NSDateComponents * qianDateComponents = [[NSDateComponents alloc] init];
        [qianDateComponents setDay:-2];
        NSDate * qianDate = [greCalendar dateByAddingComponents:qianDateComponents toDate:[NSDate date] options:0];
        NSDateComponents * qianDateComponents1 = [greCalendar components:unitFlags fromDate:qianDate];
        if (dateComponents1.year==dateComponents.year && dateComponents1.month==dateComponents.month && qianDateComponents1.day == dateComponents.day) {
            destDateString = @"前天";
        }
...
+ (UIImageView *)drawDashLineWithRect:(CGRect)rect WithColor:(UIColor *)color parentView:(UIView *)parentView
{
    ...
    CGContextMoveToPoint(line, 0, 0.5);    //开始画线
    CGContextAddLineToPoint(line, rect.size.width, 0.5);
